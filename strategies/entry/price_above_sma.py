...
class PriceAboveSMA(BaseEntryStrategy):
    ...
    Parameters:
        sma_period: Period for SMA calculation -> In new version use sma_column
    ...
    def __init__(self, params: dict = None):
        ...
        # self.sma_period = self.params.get("sma_column")
        self.lookback = self.params.get("lookback")
        # self.sma_column = f"{self.sma_period}"
        self.sma_column = self.params.get("sma_column")

        logger.info(
            # f"Initialized PriceAboveSMA: period={self.sma_period}, lookback={self.lookback}"
            f"Initialized PriceAboveSMA: period={self.sma_column}, lookback={self.lookback}"
        )
    ...
    def should_enter(self, data) -> bool:
        ...
        try:
            ...
            if current_close > current_sma and prev_close <= prev_sma:
                logger.info(
                    f"Entry signal: Crossover detected - {prev_close:.4f}≤{prev_sma:.4f} → {current_close:.4f}>{current_sma:.4f}"
                )
                return True
